...
#define CHAR_EVENT_MAX TAB5_CHAR_EVENT_MAX_TEXT
...
static tab5_keyboard_event_cb_t s_event_callback = NULL;
...
/* Converte o protocolo Character em um evento independente da UI. */
static void emit_event(const char *str, uint8_t str_len, uint8_t modifier)
{
    ...
    if (entry == nullptr) {
        ...
        if (copy == 1 && buf[0] != 0 && (uint8_t)buf[0] >= 0x20 && (uint8_t)buf[0] != 0x7F) {
            if (s_event_callback != NULL) {
                s_event_callback(buf, 1, modifier, 0);
            }
        } else {
    ...
    case TAB5_KEY_CHAR:
        if (modifier != 0) {
            ...
            if (s_event_callback != NULL) {
                s_event_callback(buf, copy, modifier, entry->ch);
            }
        } else {
            if (s_event_callback != NULL) {
                s_event_callback(buf, copy, 0, entry->ch);
            }
        }
        break;
    case TAB5_KEY_SPECIAL:
        if (s_event_callback != NULL) {
            s_event_callback(NULL, 0, modifier, entry->lvgl_key);
        }
        break;
...
static void drain_events(void)
{
    ...
    for (uint8_t i = 0; i < count && i < 32; i++) {
        ...
        /* O comprimento inclui o byte de modificador. Leia exatamente esse
         * comprimento; os bytes de texto ocupam apenas [1, len). */
        uint8_t raw[256];
        if (kb_read_bytes(REG_CHAR_EVENT_BASE, raw, len) != ESP_OK) {
            break;
        }

        tab5_char_event_t event = {};
        if (tab5_char_event_parse(raw, len, &event)) {
            emit_event(event.text, event.length, event.modifier);
        }
    }
...
{
    (void)arg;
    s_task_running.store(true, std::memory_order_release);
    uint32_t last_detect = 0;
    while (!s_stop_task.load(std::memory_order_acquire)) {
        uint32_t dummy = 0;
        TickType_t wait = pdMS_TO_TICKS(EVENT_POLL_INTERVAL_MS);
        ...
        if (s_stop_task.load(std::memory_order_acquire)) {
            break;
        }

        /* O IRQ é o caminho normal, mas algumas revisões do teclado mantêm
         * INT baixo. O polling garante que os eventos ainda sejam lidos. */
    ...
    s_task_running.store(false, std::memory_order_release);
    vTaskDelete(NULL);
}
...
static bool detect_change(void)
{
    ...
    const bool was_connected = s_connected.load(std::memory_order_acquire);
    if (present != was_connected) {
        s_connected.store(present, std::memory_order_release);
...
esp_err_t tab5_keyboard_init(void)
{
    ...
    s_stop_task.store(false, std::memory_order_release);
...
{
    if (s_task != NULL) {
        s_stop_task.store(true, std::memory_order_release);
        uint32_t wake = 1;
        if (s_intr_queue != NULL) {
            xQueueSend(s_intr_queue, &wake, 0);
        }
        for (int i = 0; i < 20 && s_task_running.load(std::memory_order_acquire); ++i) {

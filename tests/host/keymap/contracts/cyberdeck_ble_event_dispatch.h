 ...
 *
 * This seam is the only place where an asynchronous BLE stack callback is
 * allowed to hand data to the UI model.  Its whole purpose is to make that hand
 * off cheap and safe: allocate nothing unbounded, copy only a bounded snapshot,
 * hand out monotonic tokens so a late callback from a superseded generation is
 * dropped instead of corrupting the visible state, and keep a bounded queue so a
 * flood of advertisements can never starve the UI.
 */
#pragma once
